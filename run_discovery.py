...
def main() -> None:
    ...
    parser.add_argument("--fee", type=float, default=0.0006, help="Round-trip transaction cost per trade")
    ...
    survivors = discover_strategies(
        df, n_strategies=args.n_strategies, seed=args.seed, fee=args.fee
    )
    save_survivors(survivors, args.output)

    print(f"Generated: {args.n_strategies} random strategies")
    print(f"Survivors: {len(survivors)}")
    print(f"Fee (round-trip): {args.fee:.5f}")
