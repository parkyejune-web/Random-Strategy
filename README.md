...
- Conservative execution assumptions: when TP and SL hit on same bar, SL is taken first.
- Transaction cost support (default round-trip fee = 0.06%).
- Robustness checks:
...
```bash
python run_discovery.py --n-strategies 3000 --seed 42 --fee 0.0006 --output survivors.csv
```
